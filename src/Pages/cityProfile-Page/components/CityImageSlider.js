...
const CityImageSlider=()=>{
        ...
        async function fetchUnsplash() {
            try {
              const response = await fetch(
                `https://api.unsplash.com/search/photos?query=${cityName}&client_id=4BZf0SyW_btl141ZgSFJridUrU9F1M5OZYQJlT5iWK8`
              );
    ...
    {unSplash.map((image, index) => (
        <div key={index}>
          <img className={classes.sliderImg} src={image} alt={`Slide ${index + 1}`} />
        </div>
      ))}
    </Slider>
    </div>
